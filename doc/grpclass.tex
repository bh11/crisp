...
In the first form, <rec> must be a record which may have components `\\in'
and `name', having the same meaning as in `Class' (see "Class"). It is the
user's responsibility to ensure that the function supplied returns the same
value when called with isomorphic groups. `GroupClass(<func>)' is a
shorthand for `GroupClass(rec(\\in := func))'.
...
GroupClass ([ <pc group of size 2 with 1 generators>, 
  <pc group of size 3 with 1 generators> ], function( G, H ) ... end)
...
`IsNonEmpty', `IsNormalSubgroupClosed' and `IsNormalProductClosed'.
...
`IsGroupClass', `IsNonEmpty',`IsQuotientClosed' and `IsResiduallyClosed'.

\>SetIsOrdinaryFormation(<class>, <bool>) F

\index{SetIsFormation}%
If <bool> is `true', this sets the attributes `IsQuotientClosed',
`IsNonEmpty', and `IsResiduallyClosed' of <class>, making it a formation.

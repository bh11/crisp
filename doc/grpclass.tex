...
depending whether <G> is in <class> or not. It is the user's responsibility to
ensure that the function supplied returns the same value when called with isomorphic
groups. If <rec> has components  `name' or  `char', their values are stored in the
attributes `Name' (see "ref:Name") and `Characteristic' (see "Characteristic") of
<class>.
...
<grpclass> for two normal subgroup $N_1$ and $N_2$ of $G$ *which intersect
...
is true of <obj> knows if it is a Fitting class, that is, if it has 
`HasIsGroupClass', `HasContainsTrivialGroup', `HasIsNormalSubgroupClosed' 
and `HasIsNormalProductClosed'.

\>IsFittingClass( <obj>) F

is true of <obj> is a Fitting class, that is, if it has `IsGroupClass',
`ContainsTrivialGroup', `IsNormalSubgroupClosed' and `IsNormalProductClosed'.
...
returns true if <obj> lies in the filters `IsOrdinaryFormation' and 
`IsFittingClass' (see "IsOrdinaryFormation" and "IsFittingClass").


\>IsFittingFormation(<obj>) 
...
returns true if <obj> lies in the filters `IsSaturatedFormation' and 
`IsFittingClass' (see "IsSaturatedFormation" and "IsFittingClass").

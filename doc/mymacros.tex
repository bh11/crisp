%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%%  mymacros.tex                  CRISP                    Burkahrd Hofling
%%
%%  This is a modified version of gapmacro.tex - this macro file
%%  automatically produces a file `manual.tst' which contains all examples.
%%  the modifications are marked %BH
%%
%%  was: gapmacro.tex             GAP manual                   Frank Celler
%A                                                           Heiko Theissen
%A                                                           Alexander Hulpke
%%
%%  gapmacro.tex,v 4.32 1999/12/07 16:04:12 ahulpke Exp $
%%
...
\catcode`#=\active \catcode`&=6 \gdef#{\string#}% BH: replaced \char35 by \string# - this also works for \write
...
%BH: begin modifications
\outer\def\beginexample{\par
  \begingroup\advance\leftskip by \ttindent
  \ttverbatim \parskip=0pt \catcode124=0 \rightskip-5pc 
  \vskip - \baselineskip \medskip \examplefinish} 
{\catcode124=0 |catcode92=\other % | is temporary escape character
  |obeylines % end of line is active
  |gdef||{|char124}%
  |gdef|examplefinish#1^^M#2\endexample{%
  |immediate|write|tstout{#1}%
  #1|par%
  |ifx|relax#2|relax%
     |def|next{|endgroup|endgroup%
       |vskip-|parskip|medskip|noindent|ignorespaces}%
		|else%
     |def|next{|examplefinish#2\endexample}%
  |fi|next}}

\newwrite\tstout
\immediate\openout\tstout=\jobname.tst
%BH: end modifications
...
\def\folio{\ifnum\pageno<0 \romannumeral-\pageno \else
  \number\pageno\fi}
\def\doindex#1#2#3{\write\indexout{\noexpand\indexentry{#1#2#3}%
  {\ifnum\pageno<0\romannumeral-\pageno \else %BH: remove space after 0
   \number\pageno\fi}}%
  \ifvmode\nobreak\fi}
...
{\catcode`@=11
 \gdef\label#1{{\catcode`_=11\def_{X}% %BH: delete space at end ofl line
 {\let\next=\relax
  \expandafter\edef\next{\def\noexpand\xpandlowr{#1}}%BH: delete space at end ofl line
  \lowercase\expandafter{\next}%BH: delete space at end ofl line
  \ifnum\secno=0 \edef\next{\the\chapno}\else
  ...
  %\immediate\write16{Label `\book:#1' has changed.}%BH: delete space at end ofl line
...
\outer\def\FrontMatter{%
  ...
  \setbox0=\vbox{\Bibliography} % BH: uncommented - need this for references
  \endgroup
  \labchangedfalse
...
\def\EndOfBook{\vfill\supereject
  ...
  \immediate\closeout\tstout
  \immediate\write16{## Examples written on \jobname.tst.}

...
\>OneInvSgrMinWrtQPropUnderAction(<act>, <grp>, <pretest>, <test>, <data>) O
...
`OneInvSgrMinWrtQPropUnderAction' computes an <act>-invariant subgroup $M \in \cal X$
...
`OneInvSgrMinWrtQPropUnderAction' (see below for examples). $<U>/<V>$ is a
...
<data> is never used or changed by `OneInvSgrMinWrtQPropUnderAction', but
...
`OneInvSgrMinWrtQPropUnderAction' will return the $\cal C$-residual of
<grp>.

The following example shows how to use `OneInvSgrMinWrtQPropUnderAction' to
...
>       x -> Comm (x[1],x[2])));;
gap> OneInvSgrMinWrtQPropUnderAction (
>    G, G,
...
\>AllInvSgrsWithQPropUnderAction(<act>, <grp>, <pretest>, <test>, <data>) O

`AllInvSgrsWithQPropUnderAction' returns a list consisting of all 
<act>-invariant subgroups in $\cal X$, where $\cal X$ is the class defined by <pretest>,
<test>, and <data>, as described for `OneInvSgrMinWrtQPropUnderAction' (see
"OneInvSgrMinWrtQPropUnderAction"). 
At present, there exist only methods for finite solvable groups <grp>.
...
`OneInvSgrMinWrtQPropUnderAction' (see
"OneInvSgrMinWrtQPropUnderAction"), where $<act> = <grp>$. 

\>AllNormalSubgroupsWithQProperty(<grp>, <pretest>, <test>, <data>) O

`AllNormalSubgroupsWithQProperty' is the same as 
`AllInvSgrsWithQPropUnderAction' (see
"AllInvSgrsWithQPropUnderAction"), where $<act> = <grp>$. 

...
Note that `Radical' (see "Radical") , which is guaranteed to work for arbitrary 
Fitting classes <class>, should be used by the user to compute
<grpclass>-radicals.

\>InjectorFunction(<class>) A

If bound, `InjectorFunction' stores a function taking a group $G$ as the
only argument and returning a <grpclass>-injector of $G$. Note that `Injector'
(see "Injector") should be used by the user to compute
<grpclass>-injectors.

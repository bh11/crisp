...
\>OneInvSgrMaxWrtNPropUnderAction(<act>, <grp>, <pretest>, <test>, <data>) O
...
`OneInvSgrMaxWrtNPropUnderAction' computes an <act>-invariant subgroup $M \in
...
this case, `OneInvSgrMaxWrtNPropUnderAction' will return the $\cal
X$-radical of <grp>.
...
`OneInvSgrMaxWrtNPropUnderAction'. $<U>/<V>$ is an <act>-composition factor of
...
<data> is never used or changed by `OneInvSgrMaxWrtNPropUnderAction', but
exists only as a means for passing additional information to or between
the functions <pretest> and <test>.

\>AllInvSgrsWithNPropUnderAction(<act>, <grp>, <pretest>, <test>, <data>) O
...
the documentation of `OneInvSgrMaxWrtNPropUnderAction' (see
"OneInvSgrMaxWrtNPropUnderAction") for details.

\beginexample
gap> D := DihedralGroup (8);;
gap> AllInvSgrsWithNPropUnderAction (
> D, D,
>     ReturnFail,
>     function (R, S, data)
>         return IsAbelian (R);
...
are the same as `OneInvSgrMaxWrtNPropUnderAction' (see
"OneInvSgrMaxWrtNPropUnderAction")
and  `AllInvSgrsWithNPropUnderAction' ((see
"AllInvSgrsWithNPropUnderAction"), where $<act> = <grp>$, and thus the

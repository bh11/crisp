			...
			if n < i then 
				...
				for x in m do
					if RelativeOrderOfPcElement (pcgs, x) = p then
						...
						AddPcElementToPcSequence (pcgs, dpcgs, depths, x);
					fi;
				od;
				dpcgs := CanonicalPcgs (
...
InstallMethod (InducedPcgsWrtPcgsOp, "sift existing pcgs",
	...
	function (grp, pcgs)
		...
		depths := [];
		seq := [];
		for x in Pcgs (grp) do
			AddPcElementToPcSequence (ppcgs, seq, depths, x);
		od;
		return InducedPcgsByPcSequenceNC (ppcgs, seq);
	end);
...
#############################################################################
##
#M  ExponentsWrtPcSequence (<pcgs>, <seq>, <depths>, <x>, <inds>)
##
InstallMethod (ExponentsWrtPcSequence, "method for pc sequence",
	function (a, b, l, c, d) return 
		IsIdenticalObj (a, CollectionsFamily (c));
	end,
	[IsPcgs, IsListOrCollection, IsList, IsMultiplicativeElementWithInverse, IsListOrCollection], 
	0,
	function (pcgs, seq, depths,  x, inds)

	local d, pos, len, tmp, exp, maxd, p2;
	
	exp := ListWithIdenticalEntries (inds, 0);
	maxd := depths[inds[Length(inds)]];
	d := DepthOfPcElement (pcgs, x);
	pos := PositionSorted (depths, d);
	len := Length (depths);
	
	while pos <= len and depths[pos] = d and d <= maxd do
		tmp := LeadingExponentOfPcElement (pcgs, x)/
			LeadingExponentOfPcElement (pcgs, seq[pos]) mod RelativeOrderOfPcElement (pcgs, x);
		x := LeftQuotient (seq[pos]^tmp, x);
		
		p2 := Position (inds, pos);
		if p2 <> fail then
			exp[p2] := tmp;
		fi;

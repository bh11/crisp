...
InstallMethod (PcgsNormalizerOfPronormalSubgroup, 
	...
	local parentpcgs, N, r, 
		...
		conj, e, v, w, 
		null, mat, sol, n, NN, sub, inters, gens, res;

	parentpcgs := ParentPcgs (pcgs);
	ind := IndicesElementaryAbelianSteps (pcgs);
	
	if ind = fail then
		Error ("pcgs must refine an elemetary abelian series");
	elif parentpcgs <> ParentPcgs (ppcgs) then
		Error ("ppcgs must be induced from pcgs");
	fi;
	
#	N := ElementaryAbelianSubseries (pcgs);	
	N := List ([1..Length (ind)], 
	...
	for i in [3..r] do
		...
		# N[i-1] mod (<ppcgs> inters N[i-1]) N[i]
